...
- [bob-harness-capabilities.md](bob-harness-capabilities.md) — Bob-port Phase 0 verification: AGENTS.md `@`-imports (depth 5), skills are IDE-only + model-activated (not user-invoked), 5 built-in modes (no Debug/Architect), `custom_modes.yaml`/`mcp.json`/`groups` schemas; subtask cold-context isolation is runtime-only (IBM Bob docs, 2026-06-23)
