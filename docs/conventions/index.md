...
- [documentation-writing.md](documentation-writing.md) — read before writing/revising human-facing docs: ground → contract → write (one page/agent) → verify
- [verification-pass.md](verification-pass.md) — how to flip a plan's UNVERIFIED items to VERIFIED before build
- [phasal-plan.md](phasal-plan.md) — the output contract for planning: what a plan must produce to be implementable
- [implementation-cycle.md](implementation-cycle.md) — the standard build loop (build → review → QA → adversarial → PR → record), one phase per pass
- [grill-me.md](grill-me.md) — style preferences for running a grilling session (load on demand when grilling)
